...
## Week 6 - M4 生产 Agent 架构设计 (2026-07-20)

**状态**: ✅ 完成 + 出关

**完成**:
- ✅ **M4-W1** 场景建模: 角色 B（诊断建议者）, 归因不一致场景, 成功标准 (效率+准确率双指标)
- ✅ **M4-W2** 工具设计: 7 工具 + 权限模型 (只读全开/写入需确认), search_code 保留决策
- ✅ **M4-W3** 架构选型: 单 Agent, 诊断流程时序图, 升级信号 (≥3独立查询 → Orchestrator)
- ✅ **M4-W4** 可靠性工程: 四层防线 (重试/工具超时/诊断超时/降级接管), 人工接管协议+置信度
- ✅ **M4-W5** 出关: 费曼三问 PASS (Q1 待精进: Agent vs 脚本区分力), Anki 6 张

**核心产出**:
- 设计文档: `design/M4-运维告警诊断Agent-设计文档.md`
- Anki 卡: `anki/M4.tsv` (6 张)

**关键决策**:
- Battle Map v2 M4 (Advanced Patterns) → 重新定义为「生产设计」, Advanced Patterns → M4.5 按需取用
- 单 Agent 而非 Orchestrator: 串行诊断链不需要并行架构
- search_code 保留: 用户推翻了"禁止查代码"的建议, 论证"仅本地"安全边界

**待强化**:
- Agent vs 脚本的本质区分——不是"能做什么", 是"谁决定下一步"

**下一步**:
- [ ] M4.5: Advanced Patterns 按需取用 / 或直接进 M5 毕业项目
